...
public class Plugin : BaseUnityPlugin
{
    ...
    public const float PluginVersion = 1.08f;
    ...
    private void Awake()
    {
        try
        {
            ...
            PersistentDataOverride = this.Config.Bind("General", "PersistentDataOverride", "",
                "Override the persistent data path, which is used for storing the cache and mappings. This is useful if you want to use a different drive for the cache and mappings, since they can get quite large. Note that this will not move the existing cache and mappings, so you will have to move them manually. Leave this empty to use the default persistent data path: " + Path.Combine(Application.persistentDataPath, "HTCCL").Replace('/', '\\'));
            ...
            UseFullQualityTextures = this.Config.Bind("General", "UseFullQualityTextures", false,
                "(EXPERIMENTAL) Allow HTCCL to use the full resolution textures without scaling them down (the game will still change the aspect ratio to fit the texture).");
...
#pragma warning restore CS0162 // Unreachable code detected
            ...
            if (GameVersion != PluginVersion)
            {
                if (GameVersion > PluginVersion)
                {
                    LogWarning($"Your game version ({GameVersion}) is newer than HTCCL's target version ({PluginVersion}). Keep in mind that it may not work as intended on this version. Check the Steam Workshop page for updates.");
                }
                else
                {
                    LogWarning($"Your game version ({GameVersion}) is older than HTCCL's target version ({PluginVersion}). Keep in mind that it may not work as intended on this version. Check for updates in the game's properties on Steam.");
                }
            }
